        ...
        try:
            ...
            if not is_major_issue:
                ...
                if now - self.last_spoken_at < 5:
                    print("Skipping because of 5-second cooldown.")
                    return None
            ...
            # The voice/coaching layer must NEVER be able to crash the workout
            # loop. If anything here fails, just skip this round of feedback --
            # rep counting, set tracking, and DB saving all happen independently
            # of this and must keep working regardless.
            print(f"[VoicePipeline] Skipping feedback due to error: {e}")
            return None
...
def queue_audio(result):
    """Store a (voice_bytes, text) result in session_state for the
    main page to pick up and play on the next render."""
    if not result:
        return
