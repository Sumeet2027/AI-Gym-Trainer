...
from services.coaching.voice_pipeline import queue_audio
...
def sync_metrics_update(context):
    ...
    if target_sets > 0 and reps_per_set > 0 and sets_completed > last_saved_sets:
        ...
        try:
            ...
            print(f"[Metrics] Failed to save exercise progress: {e}")

        if st.session_state.get("voice_pipeline") and st.session_state.get("voice_coaching_enabled", True):
    ...
    if not pose_detected and st.session_state.get("voice_pipeline") and st.session_state.get("voice_coaching_enabled", True) and not workout_completed:
        result = st.session_state.voice_pipeline.process_event(
    ...
    if st.session_state.get("voice_pipeline") and st.session_state.get("voice_coaching_enabled", True) and not workout_completed:
        result = st.session_state.voice_pipeline.process_event(
